...
class BookMoreThanPointsTest(unittest.TestCase):
    ...
    @patch('server.clubs', [
        {"name": "Mocked Club", "email": "mocked@club.com", "points": "10"},
    ])
    ...
    def test_purchase_places_not_enough_points(self):
        ...
        with self.app as client:
            ...
            response = client.post('/purchasePlaces', data={'competition': 'Mocked Competition', 'club': 'Mocked Club', 'places': '15'})

        # Check if the flash message contains the expected text
        with app.test_request_context():
            self.assertIn(b"Not enough points in this club to book", response.data)
